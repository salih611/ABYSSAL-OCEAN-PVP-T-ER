...
const TRANSLATIONS: Record<LangKey, Record<string, string>> = {
  tr: {
    ...
    intro_p1_a: "Gelişmekte olan", intro_p1_b: "Minecraft tier sunucumuz",
    intro_p1_c: "'da,", intro_p1_d: "8 farklı kit",
    intro_p1_e: "ile kendini test edip", intro_p1_f: "gerçek PvP seviyeni",
    intro_p1_g: "hemen öğrenebilirsin.",
    intro_p2_a: "Üstelik yeteneğine güveniyorsan sunucumuzda",
    intro_p2_b: "Tester", intro_p2_c: "olabilir ya da yönetim kadromuza katılarak",
    intro_p2_d: "yetkili", intro_p2_e: "olarak yer alabilirsin.",
    ...
    installApp: "📲 Ana Ekrana Ekle", installTitle: "Abyssal Ocean'ı Yükle",
    installDesc: "Ana ekranına ekleyerek hızlı erişim sağla!",
  ...
  en: {
    ...
    intro_p1_a: "On our developing", intro_p1_b: "Minecraft tier server",
    intro_p1_c: ",", intro_p1_d: "8 different kits",
    intro_p1_e: "let you test yourself and discover your", intro_p1_f: "true PvP level",
    intro_p1_g: "instantly.",
    intro_p2_a: "If you trust your skills, you can become a",
    intro_p2_b: "Tester", intro_p2_c: "on our server, or join our management team as a",
    intro_p2_d: "staff member", intro_p2_e: ".",
    ...
    installApp: "📲 Add to Home Screen", installTitle: "Install Abyssal Ocean",
  ...
  de: {
    ...
    intro_p1_a: "Auf unserem aufstrebenden", intro_p1_b: "Minecraft-Tier-Server",
    intro_p1_c: "kannst du dich mit", intro_p1_d: "8 verschiedenen Kits",
    intro_p1_e: "testen und dein", intro_p1_f: "echtes PvP-Level",
    intro_p1_g: "sofort herausfinden.",
    intro_p2_a: "Wenn du deinen Fähigkeiten vertraust, kannst du auf unserem Server",
    intro_p2_b: "Tester", intro_p2_c: "werden oder unserem Management-Team als",
    intro_p2_d: "Mitarbeiter", intro_p2_e: "beitreten.",
    ...
    installApp: "📲 Zum Startbildschirm", installTitle: "Abyssal Ocean installieren",
    installDesc: "Füge es deinem Startbildschirm hinzu!",
    installBtn: "Installieren", installDismiss: "Später",
...
const KITS: Record<string, { ad: string; icon: JSX.Element; color: string; description: Record<LangKey, string>; detail: Record<LangKey, string> }> = {
  ...
  sword: {
    ...
    detail: { tr: "Klasik kılıç savaşı! Combo, timing ve crit vuruşların ustalığını sergile.", en: "Classic sword battle!", de: "Klassischer Schwertkampf!" }
  },
  axe: {
    ...
    detail: { tr: "Yüksek hasar, kalkan kırma!", en: "High damage, shield breaking!", de: "Hoher Schaden, Schildbruch!" }
  },
  nethpot: {
  ...
  pot: {
    ...
    detail: { tr: "Şifa iksirleri, hız ve güç potions!", en: "Healing potions, speed and power potions!", de: "Heiltränke, Geschwindigkeit!" }
  },
  uhc: {
    ...
    detail: { tr: "Doğal yenilenme yok! Golden apple ile hayatta kal.", en: "No natural regen!", de: "Keine natürliche Regeneration!" }
  },
  smp: {
    ...
    detail: { tr: "Gerçek survival deneyimi!", en: "Real survival experience!", de: "Echtes Survival-Erlebnis!" }
  },
  mace: {
    ...
    detail: { tr: "1.21'in yeni efsanevi silahı!", en: "The legendary new weapon of 1.21!", de: "Die legendäre neue Waffe von 1.21!" }
  },
};
...
const getHighestTier = (tiers: Record<string, string>): string => {
  ...
  for (const tier of Object.values(tiers || {})) {
    ...
    if (cleaned && order[cleaned] !== undefined && order[cleaned] > highestValue) {
      highestValue = order[cleaned]; highestTier = cleaned;
    }
  }
  return highestTier || "—";
};
...
const Bubbles = () => {
  ...
  return (
    <div className="fixed inset-0 pointer-events-none overflow-hidden z-0">
      {bubbles.map(bubble => (
        <div key={bubble.id} className="absolute bottom-0 rounded-full" style={{
          ...
          animation: `bubbleFloat ${bubble.duration}s ease-in infinite`, animationDelay: `${bubble.delay}s`,
        }} />
      ))}
    </div>
  );
...
const SearchWithSuggestions = ({ players, searchQuery, setSearchQuery, onSelectPlayer, theme, t, setCurrentPage }: {
  players: Player[]; searchQuery: string; setSearchQuery: (q: string) => void;
  onSelectPlayer: (p: Player) => void; theme: typeof THEMES[ThemeKey];
  t: (key: string) => string; setCurrentPage: (p: PageType) => void;
}) => {
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    ...
    if (e.key === "ArrowDown") { e.preventDefault(); setHighlightIdx(prev => Math.min(prev + 1, suggestions.length - 1)); }
      ...
      e.preventDefault();
      onSelectPlayer(suggestions[highlightIdx]);
  ...
  return (
    <div ref={wrapperRef} className="relative">
      <input type="text" placeholder={t("searchPlayer")} value={searchQuery}
      ...
      <AnimatePresence>
        {showDropdown && (
          <motion.div initial={{ opacity: 0, y: -5, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -5, scale: 0.98 }}
            className="absolute left-0 right-0 top-full mt-1 rounded-xl shadow-2xl overflow-hidden z-[60] max-h-[360px] overflow-y-auto"
            style={{ background: theme.cardBg, border: `1px solid ${theme.border}` }}>
            {suggestions.length === 0 ? (
              <div className="px-4 py-6 text-center text-sm" style={{ color: theme.textMuted }}>
                <span className="text-2xl block mb-2">🔍</span>{t("noResults")}
              </div>
            ) : (
              suggestions.map((player, idx) => (
                <button key={player.id}
                  onMouseEnter={() => setHighlightIdx(idx)}
                  onClick={() => { onSelectPlayer(player); setCurrentPage("rankings"); setFocused(false); setSearchQuery(""); }}
                  className={`w-full flex items-center gap-3 px-4 py-3 text-left transition-all ${highlightIdx === idx ? "bg-white/10" : "hover:bg-white/5"}`}
                  style={highlightIdx === idx ? { boxShadow: `inset 3px 0 0 ${theme.primary}` } : {}}>
                  <img src={player.avatar} alt="" className="w-8 h-8 rounded-lg flex-shrink-0" onError={(e) => { (e.target as HTMLImageElement).src = `https://mc-heads.net/avatar/Steve/32`; }} />
                  <div className="flex-1 min-w-0">
                    <div className="text-sm font-bold truncate" style={{ color: theme.text }}>{player.minecraftNick || player.username}</div>
                    <div className="flex items-center gap-1.5 mt-0.5">
                      <span className={`text-[9px] font-black px-1.5 py-0.5 rounded bg-gradient-to-r ${REGIONS[player.region]?.color} text-white`}>{REGIONS[player.region]?.code}</span>
                      <span className="text-[10px] font-bold" style={{ color: theme.primary }}>{player.totalPoints}p</span>
                      <span className="text-[10px]" style={{ color: theme.textMuted }}>#{player.rank}</span>
                    </div>
                  </div>
                  <svg className="w-4 h-4 flex-shrink-0 opacity-30" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" /></svg>
                </button>
              ))
            )}
          </motion.div>
...
/* ── Mobil Arama (Suggestions ile) ── */
const MobileSearchWithSuggestions = ({ players, searchQuery, setSearchQuery, onSelectPlayer, theme, t, setCurrentPage }: {
  players: Player[]; searchQuery: string; setSearchQuery: (q: string) => void;
  onSelectPlayer: (p: Player) => void; theme: typeof THEMES[ThemeKey];
  t: (key: string) => string; setCurrentPage: (p: PageType) => void;
}) => {
  const suggestions = useMemo(() => {
    ...
    const q = searchQuery.toLowerCase();
    return players.filter(p =>
      p.username?.toLowerCase().includes(q) || p.minecraftNick?.toLowerCase().includes(q)
    ).slice(0, 6);
  }, [players, searchQuery]);
      ...
      <AnimatePresence>
        {searchQuery.length >= 1 && (
          <motion.div initial={{ opacity: 0, y: -5 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -5 }}
            className="absolute left-0 right-0 top-full mt-1 rounded-xl shadow-2xl overflow-hidden z-[60] max-h-[280px] overflow-y-auto"
            style={{ background: theme.cardBg, border: `1px solid ${theme.border}` }}>
            {suggestions.length === 0 ? (
              <div className="px-4 py-4 text-center text-sm" style={{ color: theme.textMuted }}>🔍 {t("noResults")}</div>
            ) : (
              suggestions.map(player => (
                <button key={player.id}
                  onClick={() => { onSelectPlayer(player); setCurrentPage("rankings"); setSearchQuery(""); }}
...
/* ── PWA Install Banner ── */
const InstallBanner = ({ theme, t }: { theme: typeof THEMES[ThemeKey]; t: (key: string) => string }) => {
  ...
  return (
    <AnimatePresence>
      <motion.div initial={{ y: 100, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 100, opacity: 0 }}
        className="fixed bottom-4 left-4 right-4 z-[90] md:left-auto md:right-4 md:max-w-sm rounded-2xl shadow-2xl overflow-hidden backdrop-blur-xl"
        style={{ background: `${theme.cardBg}f5`, border: `1px solid ${theme.primary}40` }}>
        <div className="p-4">
          <div className="flex items-start gap-3">
            <div className={`w-12 h-12 rounded-xl bg-gradient-to-br ${theme.primaryGradient} flex items-center justify-center flex-shrink-0 text-2xl`}>
              🌊
            </div>
            <div className="flex-1 min-w-0">
              <h4 className="font-bold text-sm" style={{ color: theme.text }}>{t("installTitle")}</h4>
              <p className="text-xs mt-0.5" style={{ color: theme.textMuted }}>
                {isIOS ? (
                  <>Safari'de <strong>Paylaş</strong> butonuna basıp <strong>"Ana Ekrana Ekle"</strong> seçin.</>
                ) : (
                  t("installDesc")
                )}
              </p>
            </div>
            <button onClick={handleDismiss} className="p-1 rounded-lg hover:bg-white/10 transition-all flex-shrink-0">
              <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" style={{ color: theme.textMuted }}>
          ...
          {!isIOS && deferredPrompt && (
            <div className="flex gap-2 mt-3">
              <button onClick={handleInstall}
                className={`flex-1 py-2.5 rounded-xl font-bold text-sm bg-gradient-to-r ${theme.primaryGradient} text-white transition-all hover:scale-105 shadow-lg flex items-center justify-center gap-2`}>
                📲 {t("installBtn")}
              </button>
              <button onClick={handleDismiss}
                className="px-4 py-2.5 rounded-xl font-medium text-sm transition-all hover:bg-white/5"
  ...
  const downloadCard = async () => {
    ...
    try {
      ...
      canvas.toBlob((blob) => {
        ...
        link.href = url; link.download = `${player.minecraftNick || player.username}-abyssal-ocean.png`;
        document.body.appendChild(link); link.click(); document.body.removeChild(link);
        URL.revokeObjectURL(url); setDownloading(false);
      }, 'image/png', 1.0);
    } catch (e) { console.error(e); setDownloading(false); alert('İndirme başarısız!'); }
  };
      ...
      <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="relative w-full max-w-md" onClick={e => e.stopPropagation()}>
              ...
              {Object.entries(KITS).slice(0, 8).map(([kitKey, kit]) => {
                ...
                return (
                  <div key={kitKey} className="rounded-lg p-2" style={{ background: theme.headerBg, border: `1px solid ${theme.border}` }}>
                    ...
                    {tier ? <span className={`block text-[9px] font-black mt-1 ${tier.startsWith("HT") ? "text-amber-400" : "text-cyan-400"}`}>{tier}</span>
        ...
        <div className="flex gap-2 mt-4">
          <button onClick={downloadCard} disabled={downloading} className={`flex-1 py-3 rounded-xl font-bold bg-gradient-to-r ${theme.primaryGradient} text-white transition-all hover:scale-105 shadow-lg flex items-center justify-center gap-2 text-sm md:text-base disabled:opacity-50`}>
            {downloading ? <><div className="w-5 h-5 rounded-full border-2 border-white border-r-transparent animate-spin"></div>İndiriliyor...</>
          ...
          <button onClick={copyLink} className="flex-1 py-3 rounded-xl font-bold transition-all hover:scale-105 shadow-lg flex items-center justify-center gap-2 text-sm md:text-base"
            ...
            {copied ? <><svg className="w-5 h-5 text-green-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>{t("linkCopied")}</>
  ...
  useEffect(() => {
    ...
    const detectDevTools = () => {
      if (window.outerWidth - window.innerWidth > 160 || window.outerHeight - window.innerHeight > 160) {
        if (!devtoolsOpen) {
          devtoolsOpen = true;
          document.body.innerHTML = `<div style="position:fixed;inset:0;background:#0a0e14;color:white;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:999999;font-family:system-ui;text-align:center;padding:20px;">
            ...
            <p style="color:rgba(255,255,255,0.6);">DevTools tespit edildi. Kapatıp yenileyin.</p>
            <button onclick="window.location.reload()" style="margin-top:30px;padding:12px 30px;background:linear-gradient(90deg,#22d3ee,#3b82f6);border:none;border-radius:12px;color:white;font-weight:bold;font-size:16px;cursor:pointer;">🔄 Yenile</button></div>`;
        }
    ...
    return () => { document.removeEventListener("contextmenu", handleContextMenu); document.removeEventListener("keydown", handleKeyDown); document.removeEventListener("dragstart", handleDragStart); clearInterval(interval); };
  }, []);
};
...
const useRateLimit = () => {
  useEffect(() => {
    ...
    window.fetch = async (...args) => {
      ...
      const now = Date.now();
      const rec = counts.get(url);
      if (rec) { if (now - rec.timestamp < 60000) { rec.count++; if (rec.count > 100) throw new Error("Rate limit"); } else counts.set(url, { count: 1, timestamp: now }); }
      else counts.set(url, { count: 1, timestamp: now });
      return origFetch.apply(window, args);
    };
    let clicks = 0; let timer: NodeJS.Timeout;
    const handleClick = () => { clicks++; if (clicks > 30) return false; clearTimeout(timer); timer = setTimeout(() => { clicks = 0; }, 1000); };
    document.addEventListener("click", handleClick);
    return () => { window.fetch = origFetch; document.removeEventListener("click", handleClick); };
  }, []);
};
...
export default function App() {
  ...
  const fetchPlayers = async () => {
    try {
      const res = await fetch(`${UPSTASH_URL}/get/players`, { headers: { Authorization: `Bearer ${UPSTASH_TOKEN}`, 'Content-Type': 'application/json' } });
      const data = await res.json();
      let raw: Player[] = [];
      if (data.result) {
        raw = JSON.parse(data.result).map((p: Player) => ({ ...p, tiers: p.tiers || {}, tests: p.tests || 0, totalPoints: calculateTotalPoints(p.tiers || {}) }));
        raw.sort((a, b) => b.totalPoints - a.totalPoints);
        let trRank = 0;
        raw = raw.map(p => p.region === "TR" ? { ...p, rank: ++trRank } : { ...p, rank: 0 });
      }
      setPlayers(raw);
    } catch { setPlayers([]); } finally { setLoading(false); }
  };

  useEffect(() => { fetchPlayers(); const i = setInterval(fetchPlayers, 30000); return () => clearInterval(i); }, []);

  useEffect(() => {
    if (!players.length) return;
    const id = new URLSearchParams(window.location.search).get('player');
    if (id) { const p = players.find(p => p.id === id || p.discordId === id); if (p) { setCurrentPage("rankings"); setSelectedPlayer(p); } }
  }, [players]);
  ...
  const filteredPlayers = useMemo(() => {
    let f = [...regionPlayers];
    if (searchQuery) f = f.filter(p => p.username?.toLowerCase().includes(searchQuery.toLowerCase()) || p.minecraftNick?.toLowerCase().includes(searchQuery.toLowerCase()));
  ...
  useEffect(() => {
    if (selectedKit !== "overall" || displayCount >= kitPlayers.length) return;
    const handleScroll = () => {
      if (window.scrollY + window.innerHeight >= document.documentElement.scrollHeight - 500 && !loadingMore) {
        setLoadingMore(true);
        setTimeout(() => { setDisplayCount(prev => Math.min(prev + 20, kitPlayers.length)); setLoadingMore(false); }, 300);
      }
    };
  ...
  useEffect(() => { if (focusedIndex >= 0) document.getElementById(`player-row-${focusedIndex}`)?.scrollIntoView({ behavior: "smooth", block: "nearest" }); }, [focusedIndex]);

  const playersByTier = useMemo(() => {
    if (selectedKit === "overall") return null;
    const g: Record<number, Player[]> = { 1: [], 2: [], 3: [], 4: [], 5: [] };
    kitPlayers.forEach(p => {
      const c = cleanTier(p.tiers[selectedKit]); if (!c) return;
      const n = c.includes("1") ? 1 : c.includes("2") ? 2 : c.includes("3") ? 3 : c.includes("4") ? 4 : 5;
      g[n].push(p);
    });
    for (let i = 1; i <= 5; i++) g[i].sort((a, b) => getTierPoints(b.tiers[selectedKit]) - getTierPoints(a.tiers[selectedKit]));
    return g;
    ...
    <div className="min-h-screen w-full relative overflow-x-hidden transition-colors duration-500" style={{ background: currentTheme.bg, color: currentTheme.text }}>
      ...
      <header className="relative z-50 sticky top-0 backdrop-blur-xl border-b transition-colors duration-500" style={{ background: `${currentTheme.headerBg}cc`, borderColor: currentTheme.border }}>
        <div className="max-w-[1600px] mx-auto px-3 md:px-4 py-3 flex items-center justify-between gap-2 md:gap-4">
          ...
          <nav className="hidden lg:flex items-center gap-1">
            {(["home", "rankings"] as PageType[]).map(page => (
              <button key={page} onClick={() => setCurrentPage(page)}
                className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${currentPage === page ? `bg-gradient-to-r ${currentTheme.primaryGradient} text-white shadow-lg` : ""}`}
                style={currentPage !== page ? { background: currentTheme.cardBg, color: currentTheme.textMuted } : {}}>
                {page === "home" ? "🏠" : "🏆"} {t(page)}
              </button>
            ))}
          </nav>
          ...
          <div className="hidden md:flex items-center gap-2">
            {currentPage === "rankings" && (
              <SearchWithSuggestions players={players} searchQuery={searchQuery} setSearchQuery={setSearchQuery}
                onSelectPlayer={setSelectedPlayer} theme={currentTheme} t={t} setCurrentPage={setCurrentPage} />
            )}
            ...
            <div className="relative">
              <button onClick={() => { setLangMenuOpen(!langMenuOpen); setThemeMenuOpen(false); }}
                ...
                <span className="text-base">{LANGUAGES[language].flag}</span><span className="text-xs font-bold">{LANGUAGES[language].code}</span>
              </button>
              <AnimatePresence>
                {langMenuOpen && (
                  <motion.div initial={{ opacity: 0, y: -10, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -10, scale: 0.95 }}
                    className="absolute right-0 top-full mt-2 w-44 rounded-xl shadow-2xl overflow-hidden z-50" style={{ background: currentTheme.cardBg, border: `1px solid ${currentTheme.border}` }}>
                    {Object.entries(LANGUAGES).map(([key, lang]) => (
                      <button key={key} onClick={() => { setLanguage(key as LangKey); setLangMenuOpen(false); }}
                        className={`w-full flex items-center gap-3 px-4 py-3 text-sm transition-all ${language === key ? `bg-gradient-to-r ${currentTheme.primaryGradient} text-white` : ""}`}
                        style={language !== key ? { color: currentTheme.text } : {}}>
                        <span className="text-xl">{lang.flag}</span><div className="flex-1 text-left"><div className="font-semibold">{lang.name}</div><div className="text-[10px] opacity-60">{lang.code}</div></div>
            ...
            <div className="relative">
              <button onClick={() => { setThemeMenuOpen(!themeMenuOpen); setLangMenuOpen(false); }}
                className="flex items-center gap-1.5 px-3 py-2.5 rounded-xl text-sm font-medium transition-all hover:scale-105"
                style={{ background: currentTheme.cardBg, border: `1px solid ${currentTheme.border}`, color: currentTheme.text }}>
                <span className="text-base">{currentTheme.emoji}</span><span className="text-xs font-bold">{currentTheme.name}</span>
              </button>
              <AnimatePresence>
                {themeMenuOpen && (
                  <motion.div initial={{ opacity: 0, y: -10, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -10, scale: 0.95 }}
                    className="absolute right-0 top-full mt-2 w-52 rounded-xl shadow-2xl overflow-hidden z-50" style={{ background: currentTheme.cardBg, border: `1px solid ${currentTheme.border}` }}>
                    ...
                    {Object.entries(THEMES).map(([key, th]) => (
                      <button key={key} onClick={() => { setTheme(key as ThemeKey); setThemeMenuOpen(false); }}
                        className={`w-full flex items-center gap-3 px-4 py-3 text-sm transition-all ${theme === key ? `bg-gradient-to-r ${th.primaryGradient} text-white` : ""}`}
                        style={theme !== key ? { color: currentTheme.text } : {}}>
                        <span className="text-xl">{th.emoji}</span>
                        <div className="flex-1 text-left"><div className="font-semibold">{th.name}</div><div className="flex gap-1 mt-1"><div className="w-3 h-3 rounded-full" style={{ background: th.primary }}></div><div className="w-3 h-3 rounded-full" style={{ background: th.accent }}></div></div></div>
                        {theme === key && <span>✓</span>}
                      </button>
                    ))}
                  </motion.div>
          ...
          <div className="flex md:hidden items-center gap-2">
            {currentPage === "rankings" && (
              <button onClick={() => setMobileSearchOpen(!mobileSearchOpen)} className="p-2.5 rounded-xl transition-all"
                style={{ background: currentTheme.cardBg, border: `1px solid ${currentTheme.border}`, color: currentTheme.text }}>
                <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
              </button>
            )}
            <button onClick={() => setMobileMenuOpen(!mobileMenuOpen)} className="p-2.5 rounded-xl transition-all"
              style={{ background: currentTheme.cardBg, border: `1px solid ${currentTheme.border}`, color: currentTheme.text }}>
        ...
        <AnimatePresence>
          {mobileSearchOpen && currentPage === "rankings" && (
            <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="md:hidden px-3 pb-3 overflow-hidden">
              <MobileSearchWithSuggestions players={players} searchQuery={searchQuery} setSearchQuery={setSearchQuery}
                onSelectPlayer={setSelectedPlayer} theme={currentTheme} t={t} setCurrentPage={setCurrentPage} />
            </motion.div>
          )}
        </AnimatePresence>
              ...
              <div className="p-3 space-y-2">
                {(["home", "rankings"] as PageType[]).map(page => (
                  <button key={page} onClick={() => { setCurrentPage(page); setMobileMenuOpen(false); }}
                    className={`w-full px-4 py-3 rounded-lg text-sm font-medium transition-all flex items-center gap-2 ${currentPage === page ? `bg-gradient-to-r ${currentTheme.primaryGradient} text-white` : ""}`}
                    style={currentPage !== page ? { background: currentTheme.headerBg, color: currentTheme.text } : {}}>
                    {page === "home" ? "🏠" : "🏆"} {t(page)}
                  </button>
                ))}
            ...
            <main className="relative z-10 max-w-[1600px] mx-auto px-3 md:px-4 py-6 md:py-12">
              ...
              <div className="text-center mb-12 md:mb-20">
                ...
                <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="text-4xl sm:text-5xl md:text-7xl lg:text-8xl font-black mb-4 md:mb-6 leading-tight">
                  <span className="inline-block animated-gradient-text">ABYSSAL OCEAN</span><br />
                  <span className="inline-block">
                    {"TIER LIST".split("").map((c, i) => <span key={i} className="inline-block tier-letter" style={{ animationDelay: `${i * 0.1}s`, color: "white" }}>{c === " " ? "\u00A0" : c}</span>)}
                  </span>
                </motion.h1>
                <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="text-base md:text-xl max-w-2xl mx-auto mb-6 md:mb-10 px-4" style={{ color: currentTheme.textMuted }}>
                  {t("chooseLevel").split(" ").map((w, i) => <span key={i} className="inline-block subtitle-word" style={{ animationDelay: `${0.5 + i * 0.1}s`, marginRight: "0.3em" }}>{w}</span>)}
                </motion.p>
              ...
              {/* Intro Card */}
              <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }} className="max-w-4xl mx-auto mb-12 md:mb-20 relative">
                <div className="relative rounded-2xl md:rounded-3xl p-6 md:p-12 overflow-hidden backdrop-blur-sm" style={{ background: `linear-gradient(135deg, ${currentTheme.cardBg}cc, ${currentTheme.bg}cc)`, border: `1px solid ${currentTheme.primary}30` }}>
                  ...
                  <div className="relative z-10">
                    ...
                    <div className="grid grid-cols-3 gap-2 md:gap-4 mt-6 md:mt-8 pt-6 md:pt-8" style={{ borderTop: `1px solid ${currentTheme.border}` }}>
                      {[{ val: "8", key: "stat_kit" }, { val: "10", key: "stat_tier" }, { val: "3", key: "stat_region" }].map(s => (
                        <div key={s.key} className="text-center">
                          <div className={`text-2xl md:text-4xl font-black bg-gradient-to-r ${currentTheme.primaryGradient} bg-clip-text text-transparent`}>{s.val}</div>
                          <div className="text-[10px] md:text-xs uppercase tracking-wider mt-1" style={{ color: currentTheme.textMuted }}>{t(s.key)}</div>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 md:gap-5">
                  {Object.entries(KITS).map(([key, kit], i) => (
                    <motion.div key={key} initial={{ opacity: 0, scale: 0.9, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} transition={{ delay: i * 0.05 }}
                      whileHover={{ scale: 1.05, y: -5 }} className="group relative rounded-2xl p-5 md:p-6 transition-all cursor-pointer overflow-hidden"
                      ...
                      <div className="relative z-10">
                        <div className="mb-3 md:mb-4 flex justify-center">
                          <div className="w-14 h-14 md:w-16 md:h-16 rounded-2xl flex items-center justify-center border-2 transition-all group-hover:scale-110" style={{ background: `linear-gradient(135deg, ${kit.color}15, ${kit.color}05)`, borderColor: `${kit.color}30` }}>{kit.icon}</div>
                        </div>
                              ...
                              <motion.tr key={player.id} id={`player-row-${idx}`}
                                ...
                                <td className="px-3 md:px-6 py-3 md:py-4">
                                  <div className="flex items-center gap-2 md:gap-4">
                                    <img src={player.avatar} alt="" className="w-10 h-10 md:w-12 md:h-12 rounded-lg md:rounded-xl ring-2 flex-shrink-0" style={{ boxShadow: `0 0 0 2px ${currentTheme.border}` }} onError={(e) => { (e.target as HTMLImageElement).src = `https://mc-heads.net/avatar/Steve/64`; }} />
                                    <div className="min-w-0 flex-1">
                                      <h3 className="font-bold text-sm md:text-lg truncate" style={{ color: currentTheme.text }}>{player.minecraftNick || player.username}</h3>
                                      <div className="flex items-center gap-1 md:gap-2 mt-0.5 md:mt-1 flex-wrap player-info-row">
                                        ...
                                        <span className="text-[9px] md:text-xs font-black px-1.5 md:px-2 py-0.5 rounded-md flex items-center gap-1" style={{ background: `${currentTheme.primary}15`, color: currentTheme.primary, border: `1px solid ${currentTheme.primary}30` }}>
                                          <span className="inline-block w-1 md:w-1.5 h-1 md:h-1.5 rounded-full animate-pulse" style={{ background: currentTheme.primary }}></span>{player.totalPoints}p
                                        </span>
                                ...
                                <td className="px-3 md:px-6 py-3 md:py-4 hidden md:table-cell">
                                  <div className="flex justify-center">
                                    <span className={`inline-flex items-center justify-center px-3 py-1.5 rounded-lg text-xs font-black bg-gradient-to-br ${REGIONS[player.region]?.color || "from-gray-500 to-gray-600"} text-white shadow-lg min-w-[44px] group-hover:scale-110 transition-transform`}>{REGIONS[player.region]?.code || player.region}</span>
                                  </div>
                                </td>
                                <td className="px-3 md:px-6 py-3 md:py-4">
                                  <div className="flex items-center justify-end gap-1 md:gap-2 flex-wrap">
                                    {Object.entries(KITS).map(([kitKey, kit]) => {
                                      const displayTier = cleanTier(player.tiers[kitKey]);
                                      return (
                                        <div key={kitKey} className="w-9 h-9 md:w-14 md:h-14 rounded-lg md:rounded-xl flex flex-col items-center justify-center transition-all hover:scale-110" style={{ background: currentTheme.headerBg, border: `1px solid ${currentTheme.border}` }}>
                                          ...
                                          {displayTier ? <span className={`text-[7px] md:text-[10px] font-black leading-none mt-0.5 md:mt-1 ${displayTier.startsWith("HT") ? "text-amber-400" : "text-cyan-400"}`}>{displayTier}</span>
                                ...
                                return (
                                  <button key={player.id} onClick={() => setSelectedPlayer(player)} className="w-full flex items-center gap-2 p-2 rounded-lg hover:bg-white/5 text-left">
                                    ...
                                    <div className="flex-1 min-w-0">
                                      ...
                                      <div className="flex items-center gap-1.5 mt-0.5">
                                        <span className={`text-[9px] md:text-[10px] px-1.5 py-0.5 rounded font-bold text-white bg-gradient-to-r ${TIER_COLORS[displayTier] || "from-gray-600 to-gray-700"}`}>{displayTier}</span>
      ...
      {selectedPlayer && !shareCardPlayer && (
        <div className="fixed inset-0 z-[100] flex items-end md:items-center justify-center p-0 md:p-4 bg-black/80 backdrop-blur-md" onClick={() => setSelectedPlayer(null)}>
          <motion.div initial={{ scale: 0.9, opacity: 0, y: 100 }} animate={{ scale: 1, opacity: 1, y: 0 }}
            className="relative w-full max-w-2xl rounded-t-3xl md:rounded-3xl shadow-2xl overflow-hidden max-h-[90vh] overflow-y-auto"
            style={{ background: `linear-gradient(135deg, ${currentTheme.cardBg}, ${currentTheme.bg})`, border: `1px solid ${currentTheme.border}` }} onClick={e => e.stopPropagation()}>
            <div className="flex items-start justify-between p-4 md:p-6 sticky top-0 backdrop-blur-xl" style={{ borderBottom: `1px solid ${currentTheme.border}`, background: `${currentTheme.cardBg}f0` }}>
              <div className="flex items-center gap-3 md:gap-4 min-w-0">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="text-xs md:text-sm mt-1 flex items-center gap-1.5 flex-wrap" style={{ color: currentTheme.textMuted }}>
                    <span className={`inline-flex items-center justify-center px-2 py-0.5 rounded text-[10px] font-black bg-gradient-to-br ${REGIONS[selectedPlayer.region]?.color} text-white shadow`}>{REGIONS[selectedPlayer.region]?.code}</span>
                    <span>#{selectedPlayer.rank}</span><span>•</span><span className="font-bold" style={{ color: currentTheme.primary }}>{selectedPlayer.totalPoints} {t("points")}</span>
                  </div>
              ...
              <div className="flex items-center gap-1.5 md:gap-2 flex-shrink-0">
                <button onClick={() => setShareCardPlayer(selectedPlayer)} className="p-2 rounded-xl transition-all hover:scale-110" style={{ background: currentTheme.primary, color: 'white' }}>
            ...
            <div className="p-4 md:p-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-2 md:gap-3">
                {Object.entries(KITS).map(([kitKey, kit]) => {
                  const displayTier = cleanTier(selectedPlayer.tiers[kitKey]);
                  const points = getTierPoints(selectedPlayer.tiers[kitKey]);
                  return (
                    <div key={kitKey} className="rounded-xl md:rounded-2xl p-3 md:p-4" style={{ background: currentTheme.headerBg, border: `1px solid ${currentTheme.border}` }}>
                      <div className="flex items-center justify-between mb-2">
                        ...
                        {displayTier ? <span className={`text-[10px] md:text-xs font-bold px-2 py-1 rounded-lg text-white bg-gradient-to-r ${TIER_COLORS[displayTier] || "from-gray-600 to-gray-700"}`}>{displayTier}</span>
              ...
              <div className="mt-4 md:mt-6 pt-4 md:pt-6 grid grid-cols-2 gap-4" style={{ borderTop: `1px solid ${currentTheme.border}` }}>
                <div className="text-center"><div className="text-xl md:text-2xl font-black">{selectedPlayer.tests}</div><div className="text-[10px] md:text-xs mt-1" style={{ color: currentTheme.textMuted }}>{t("totalTests")}</div></div>
                <div className="text-center"><div className="text-xl md:text-2xl font-black">{getHighestTier(selectedPlayer.tiers)}</div><div className="text-[10px] md:text-xs mt-1" style={{ color: currentTheme.textMuted }}>{t("highestTier")}</div></div>
              </div>
            </div>
          </motion.div>
        </div>
      ...
      {/* ── PWA Install Banner ── */}
      <InstallBanner theme={currentTheme} t={t} />
        ...
        <div className="max-w-[1600px] mx-auto px-4 py-6 md:py-8">
          <div className="flex flex-col md:flex-row items-center justify-between gap-4 text-center md:text-left">
            ...
            <div className="flex items-center gap-3 md:gap-4 text-xs md:text-sm flex-wrap justify-center" style={{ color: currentTheme.textMuted }}>
              <a href="https://discord.gg/cKFwKcfcWn" target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5" style={{ color: currentTheme.primary }}>
                <DiscordIcon className="w-4 h-4" /> Discord
              </a>
              <span style={{ opacity: 0.3 }}>•</span><span>{t("pvpTierList")}</span>
            </div>
            <div className="text-[10px] md:text-xs" style={{ color: currentTheme.textMuted }}>
              © {new Date().getFullYear()} Abyssal Ocean. {t("allRights")}.<br /><span style={{ opacity: 0.6 }}>{t("madeWith")}</span>
            </div>
          </div>
        </div>
      </footer>
      ...
      <style>{`
        ...
        @keyframes bubbleFloat{0%{transform:translateY(0) scale(.3);opacity:0}15%{opacity:.8}85%{opacity:.5}100%{transform:translateY(-110vh) scale(1);opacity:0}}
        @keyframes gradientShift{0%,100%{background-position:0% 50%}50%{background-position:100% 50%}}
        .animated-gradient-text{background:linear-gradient(90deg,#22d3ee 0%,#3b82f6 25%,#a855f7 50%,#ec4899 75%,#22d3ee 100%);background-size:200% auto;-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;animation:gradientShift 4s ease-in-out infinite;filter:drop-shadow(0 0 30px rgba(34,211,238,.4))}
        @keyframes letterDrop{0%{transform:translateY(-30px) rotate(-10deg);opacity:0}50%{transform:translateY(5px) rotate(2deg);opacity:1}100%{transform:translateY(0) rotate(0);opacity:1}}
        @keyframes letterGlow{0%,100%{text-shadow:0 0 10px rgba(34,211,238,.5)}50%{text-shadow:0 0 20px rgba(168,85,247,.7)}}
        .tier-letter{display:inline-block;animation:letterDrop .6s cubic-bezier(.34,1.56,.64,1) backwards,letterGlow 3s ease-in-out infinite;transition:all .3s ease}
        .tier-letter:hover{transform:translateY(-10px) scale(1.2);color:#22d3ee!important}
        @keyframes slideInBadge{0%{opacity:0;transform:translateX(-10px)}100%{opacity:1;transform:translateX(0)}}
        .player-info-row>*{animation:slideInBadge .4s ease-out backwards}
        .player-info-row>*:nth-child(1){animation-delay:.05s}.player-info-row>*:nth-child(2){animation-delay:.15s}.player-info-row>*:nth-child(3){animation-delay:.25s}.player-info-row>*:nth-child(4){animation-delay:.35s}
        .player-info-row>*:hover{transform:translateY(-2px) scale(1.05);transition:transform .2s ease}
        @keyframes skeletonShimmer{0%{opacity:.3}50%{opacity:.7}100%{opacity:.3}}
        ...
        @keyframes badgeGlow{0%,100%{box-shadow:0 0 20px rgba(34,211,238,.3)}50%{box-shadow:0 0 40px rgba(34,211,238,.6)}}
        .hero-badge{animation:badgeFloat 3s ease-in-out infinite,badgeGlow 2s ease-in-out infinite}
        .hero-badge-text{background:linear-gradient(90deg,#22d3ee,#a855f7,#ec4899,#22d3ee);background-size:200% auto;-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;animation:gradientShift 3s linear infinite;font-weight:800}
        @keyframes wordFadeIn{0%{opacity:0;transform:translateY(20px) scale(.8);filter:blur(10px)}100%{opacity:1;transform:translateY(0) scale(1);filter:blur(0)}}
        .subtitle-word{animation:wordFadeIn .6s cubic-bezier(.22,1,.36,1) backwards;transition:all .3s ease;display:inline-block}
        ...
        @media(max-width:768px){body{overflow-x:hidden}.animated-gradient-text{filter:drop-shadow(0 0 15px rgba(34,211,238,.4))}}
